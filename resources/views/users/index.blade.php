...
<div class="overflow-x-auto rounded bg-white shadow">
    <table class="w-full text-sm">
        ...
        @foreach($users as $user)
            <tr class="border-t"><td class="p-3">{{ $user->name }}</td><td class="p-3">{{ $user->email }}</td><td class="p-3">{{ $user->role_name ? ucfirst($user->role_name) : '-' }}</td><td class="p-3 text-right"><a class="text-amber-600" href="{{ route('users.edit', $user) }}">Edit</a> <form class="inline" method="post" action="{{ route('users.destroy', $user) }}">@csrf @method('DELETE')<button class="text-rose-600" onclick="return confirm('Hapus user?')">Hapus</button></form></td></tr>
        @endforeach
        </tbody>
    </table>
</div>
